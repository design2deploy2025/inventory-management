  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] p-6">
      ...
      <div className="relative mb-8">
        {/* Gradient Banner */}
        <div className="h-48 rounded-2xl bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 relative overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-end p-8">
            <div className="flex items-end gap-6">
              {/* Logo */}
              <div className="relative">
                <div className="w-28 h-28 rounded-2xl bg-gradient-to-br from-white/20 to-white/5 backdrop-blur-xl border border-white/30 flex items-center justify-center overflow-hidden shadow-2xl">
                  {logoPreview ? (
      ...
      {success && (
        <div className="mb-6 p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-3 animate-fade-in">
          ...
          <span className="text-emerald-400 font-medium">Profile saved successfully!</span>
        </div>
      )}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="flex gap-2 p-1.5 bg-gray-900/50 rounded-xl backdrop-blur-sm">
            {[
              ...
              { id: 'social', label: 'Social', icon: 'M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z' }
            ].map((tab) => (
              <button
                key={tab.id}
          ...
          {/* Form Card */}
          <form onSubmit={handleSave} className="bg-gray-900/50 backdrop-blur-xl rounded-2xl border border-white/5 p-6">
            {/* Business Section */}
            {activeSection === 'business' && (
              <div className="space-y-6 animate-fade-in">
                <div>
                  <h3 className="text-lg font-semibold text-white mb-1">Business Information</h3>
                  <p className="text-sm text-slate-400 mb-6">Tell customers about your business</p>
            ...
            {activeSection === 'contact' && (
              <div className="space-y-6 animate-fade-in">
                <div>
                  ...
                  <p className="text-sm text-slate-400 mb-6">How customers can reach you</p>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">This number will be shared with customers for orders</p>
                    </div>
            ...
            {activeSection === 'social' && (
              <div className="space-y-6 animate-fade-in">
                <div>
                  ...
                  <p className="text-sm text-slate-400 mb-6">Connect your social profiles</p>
                  
                  <div>
                    <label className="block text-sm font-medium text-slate-300 mb-2">Instagram Handle</label>
                    <div className="relative group">
                      <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-emerald-400 font-semibold">
                  {Math.round(((!!formData.business_name) + (!!formData.owner_name) + (!!formData.whatsapp_number) + (!!formData.instagram_handle) + (!!formData.description) + (!!formData.address) + (!!logoPreview)) / 7 * 100)}%
                </span>
              </div>
              <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-gradient-to-r from-emerald-400 to-teal-500 rounded-full transition-all duration-500"
                  style={{ width: `${((!!formData.business_name) + (!!formData.owner_name) + (!!formData.whatsapp_number) + (!!formData.instagram_handle) + (!!formData.description) + (!!formData.address) + (!!logoPreview)) / 7 * 100}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-white/5 space-y-3">
              {[
                ...
                <div key={idx} className="flex items-center gap-3">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center ${item.filled ? 'bg-emerald-500/20' : 'bg-gray-800'}`}>
                    {item.filled ? (
                  ...
                  <span className={`text-sm ${item.filled ? 'text-slate-300' : 'text-slate-500'}`}>{item.label}</span>
                </div>
              ))}
            </div>
          </div>
